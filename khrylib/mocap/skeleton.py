from khrylib.utils.transformation import euler_matrix
...
class Skeleton:
    ...
    def load_from_offsets(self, offsets, parents, scale, exclude_bones=None, channels=None, spec_channels=None):
        if channels is None:
            channels = ['x', 'y', 'z']
        if exclude_bones is None:
            exclude_bones = {}
    ...
    def write_xml(self, fname, template_fname, offset=np.array([0, 0, 0]), ref_angles=None):
