...
if __name__ == "__main__":
    # Example usage
    # data = Struct(format)# data.name should be valid/
    def entry_callback(data):
        print(f"Entered state: {data}")

    def exit_callback(data):
            print(f"Exited state: {data}")

    sm = StateMachine( osl=1)
    state1 = State(name="state1")
    state2 = State(name="state2")

    state1.on_entry(entry_callback)
    state1.on_exit(exit_callback)
    ...
    transition1 = sm.add_transition(
        source=state1, destination=state2, event=event1, callback=None
    )
    transition2 = sm.add_transition(
        source=state2, destination=state1, event=event2, callback=None
    )
